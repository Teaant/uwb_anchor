...
typedef struct
{
    ...
    uint16		rxPreamCountNOSAT;	// Unsaturated count of preamble symbols accumulated
 ...
 * return int (1 or 0)
 */
int dwt_getgpiovalue(uint32 , UWBPortTypeDef *pports);
 ...
 * no return value
 */
void dwt_setrxantennadelay(uint16 antennaDly, UWBPortTypeDef *pports);
 ...
 * no return value
 */
void dwt_settxantennadelay(uint16 antennaDly, UWBPortTypeDef *pports);
 ...
 * no return value
 */
void dwt_enableframefilter(uint16 bitmask, UWBPortTypeDef *pports);
 ...
 * no return value
 */
void dwt_readaccdata(uint8 *buffer, uint16 length, uint16 rxBufferOffset, UWBPortTypeDef *pports);
...
#define dwt_write32bitreg(x,y,z)  dwt_write32bitoffsetreg(x,0,y,z)
#define dwt_read32bitreg(x,z)     dwt_read32bitoffsetreg(x,0,z)
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
int writetospi(uint16 headerLength, const uint8 *headerBuffer, uint32 bodylength, const uint8 *bodyBuffer, UWBPortTypeDef *pports);
 ...
 */
int readfromspi(uint16 headerLength, const uint8 *headerBuffer, uint32 readlength, uint8 *readBuffer, UWBPortTypeDef *pports);
 ...
 */
decaIrqStatus_t decamutexon(UWBPortTypeDef *pports) ;
 ...
 * returns the state of the DW1000 interrupt
 */
void decamutexoff(decaIrqStatus_t s, UWBPortTypeDef *pports) ;
