...
void _dwt_enableclocks(int clocks, UWBPortTypeDef *pports) ;
// Configure the ucode (FP algorithm) parameters
void _dwt_configlde(int prf, UWBPortTypeDef *pports);
// Load ucode from OTP/ROM
void _dwt_loaducodefromrom(UWBPortTypeDef *pports);
// Read non-volatile memory
uint32 _dwt_otpread(uint16 address, UWBPortTypeDef *pports);
// Program the non-volatile memory
uint32 _dwt_otpprogword32(uint32 data, uint16 address, UWBPortTypeDef *pports);
// Upload the device configuration into always on memory
void _dwt_aonarrayupload(UWBPortTypeDef *pports);
...
int dwt_initialise(int config, UWBPortTypeDef *pports)
{
...
pdw1000local->otp_mask = config ; // Save the READ_OTP config mask
	...
	{
		dwt_softreset(pports); // Make sure the device is completely reset before starting initialisation
	}

	if(!((DWT_DW_WAKE_UP & config) && ((DWT_READ_OTP_TMP | DWT_READ_OTP_BAT | DWT_READ_OTP_LID | DWT_READ_OTP_PID | DWT_DW_WUP_RD_OTPREV)& config)))
	{
		_dwt_enableclocks(FORCE_SYS_XTI, pports); // NOTE: set system clock to XTI - this is necessary to make sure the values read by _dwt_otpread are reliable
	}                                  // when not reading from OTP, clocks don't need to change.

	// Configure the CPLL lock detect
	dwt_write8bitoffsetreg(EXT_SYNC_ID, EC_CTRL_OFFSET, EC_CTRL_PLLLCK, pports);
	...
	if(!(DWT_DW_WAKE_UP & config))
	{
		...
		ldo_tune = _dwt_otpread(LDOTUNE_ADDRESS, pports);
		if((ldo_tune & 0xFF) != 0)
		{
			// Kick LDO tune
			dwt_write8bitoffsetreg(OTP_IF_ID, OTP_SF, OTP_SF_LDO_KICK, pports); // Set load LDO kick bit
			pdw1000local->sleep_mode |= AON_WCFG_ONW_LLDO; // LDO tune must be kicked at wake-up
		}
	}
	else
	{   //if LDOTUNE reg contains value different from default it means it was kicked from OTP and thus set AON_WCFG_ONW_LLDO.
		if(dwt_read32bitoffsetreg(RF_CONF_ID, LDOTUNE, pports) != LDOTUNE_DEFAULT)
	...
	if((!(DWT_DW_WAKE_UP & config)) || ((DWT_DW_WAKE_UP & config) && (DWT_DW_WUP_RD_OTPREV & config)))
	{
		...
		otp_xtaltrim_and_rev = _dwt_otpread(XTRIM_ADDRESS, pports) & 0xffff;        // Read 32 bit value, XTAL trim val is in low octet-0 (5 bits)
	...
	if(!(DWT_DW_WAKE_UP & config))
	{
		...
		// Configure XTAL trim
		dwt_setxtaltrim((uint8)otp_xtaltrim_and_rev, pports);
	}

	if(DWT_READ_OTP_PID & config)
	{
		// Load Part from OTP
		pdw1000local->partID = _dwt_otpread(PARTID_ADDRESS, pports);
	}
	else
	{
	...
	if(DWT_READ_OTP_LID & config)
	{
		...
		pdw1000local->lotID = _dwt_otpread(LOTID_ADDRESS, pports);
	}
	else
	{
	...
	{
		// Load VBAT from OTP
		pdw1000local->vBatP = _dwt_otpread(VBAT_ADDRESS, pports) & 0xff;
	}
	else
	{
	...
	{
		// Load TEMP from OTP
		pdw1000local->tempP = _dwt_otpread(VTEMP_ADDRESS, pports) & 0xff;
	}
	else
	{
	...
	if(!(DWT_DW_WAKE_UP & config))
	{
		...
		{
			_dwt_loaducodefromrom(pports);
			pdw1000local->sleep_mode |= AON_WCFG_ONW_LLDE; // microcode must be loaded at wake-up if loaded on initialisation
		}
		else // Should disable the LDERUN bit enable if LDE has not been loaded
		{
			uint16 rega = dwt_read16bitoffsetreg(PMSC_ID, PMSC_CTRL1_OFFSET+1, pports) ;
			rega &= 0xFDFF ; // Clear LDERUN bit
			dwt_write16bitoffsetreg(PMSC_ID, PMSC_CTRL1_OFFSET+1, rega, pports) ;
		}
	}
	...
	_dwt_enableclocks(ENABLE_ALL_SEQ, pports); // Enable clocks for sequencing

	// The 3 bits in AON CFG1 register must be cleared to ensure proper operation of the DW1000 in DEEPSLEEP mode.
	dwt_write8bitoffsetreg(AON_ID, AON_CFG1_OFFSET, 0x00, pports);

	// Read system register / store local copy
	pdw1000local->sysCFGreg = dwt_read32bitreg(SYS_CFG_ID, pports); // Read sysconfig register
	pdw1000local->longFrames = (pdw1000local->sysCFGreg & SYS_CFG_PHR_MODE_11) >> SYS_CFG_PHR_MODE_SHFT ; //configure longFrames

	pdw1000local->txFCTRL = dwt_read32bitreg(TX_FCTRL_ID, pports) ;
 ...
 * no return value
 */
void dwt_setfinegraintxseq(int enable, UWBPortTypeDef *pports)
{
    if (enable)
    {
        dwt_write16bitoffsetreg(PMSC_ID, PMSC_TXFINESEQ_OFFSET, PMSC_TXFINESEQ_ENABLE, pports);
    }
    else
    {
        dwt_write16bitoffsetreg(PMSC_ID, PMSC_TXFINESEQ_OFFSET, PMSC_TXFINESEQ_DISABLE, pports);
    }
}
 ...
 * no return value
 */
void dwt_setlnapamode(int lna_pa, UWBPortTypeDef *pports)
{
    uint32 gpio_mode = dwt_read32bitoffsetreg(GPIO_CTRL_ID, GPIO_MODE_OFFSET, pports);
    ...
    dwt_write32bitoffsetreg(GPIO_CTRL_ID, GPIO_MODE_OFFSET, gpio_mode, pports);
}
 ...
 * no return value
 */
void dwt_enablegpioclocks(UWBPortTypeDef *pports)
{
    uint32 pmsc_clock_ctrl = dwt_read32bitreg(PMSC_ID, pports);
    dwt_write32bitreg(PMSC_ID, pmsc_clock_ctrl | PMSC_CTRL0_GPCE | PMSC_CTRL0_GPRN, pports) ;
}
 ...
 * no return value
 */
void dwt_setgpiodirection(uint32 gpioNum, uint32 direction, UWBPortTypeDef *pports)
{
    ...
    dwt_writetodevice(GPIO_CTRL_ID, GPIO_DIR_OFFSET, GPIO_DIR_LEN, buf, pports);
}
 ...
 * no return value
 */
void dwt_setgpiovalue(uint32 gpioNum, uint32 value, UWBPortTypeDef *pports)
{
    ...
    dwt_writetodevice(GPIO_CTRL_ID, GPIO_DOUT_OFFSET, GPIO_DOUT_LEN, buf, pports);
}
 ...
 * return int (1 or 0)
 */
int dwt_getgpiovalue(uint32 gpioNum, UWBPortTypeDef *pports)
{
    return ((dwt_read32bitoffsetreg(GPIO_CTRL_ID, GPIO_RAW_OFFSET, pports) & gpioNum)? 1 : 0);
}
 ...
 */
uint32 dwt_readdevid(UWBPortTypeDef *pports)
{
    return dwt_read32bitoffsetreg(DEV_ID_ID,0,pports);
}
 ...
 * no return value
 */
void dwt_configuretxrf(dwt_txconfig_t *config, UWBPortTypeDef *pports)
{

    // Configure RF TX PG_DELAY
    dwt_write8bitoffsetreg(TX_CAL_ID, TC_PGDELAY_OFFSET, config->PGdly, pports);

    // Configure TX power
    dwt_write32bitreg(TX_POWER_ID, config->power, pports);
 ...
 * no return value
 */
void dwt_configurefor64plen(int prf, UWBPortTypeDef *pports)
{
    dwt_write8bitoffsetreg(CRTR_ID, CRTR_GEAR_OFFSET, DEMOD_GEAR_64L, pports);

    if(prf == DWT_PRF_16M)
    {
        dwt_write8bitoffsetreg(DRX_CONF_ID, DRX_TUNE2_OFFSET+2, DRX_TUNE2_UNCONF_SFD_TH_PRF16, pports);
    }
    else
    {
        dwt_write8bitoffsetreg(DRX_CONF_ID, DRX_TUNE2_OFFSET+2, DRX_TUNE2_UNCONF_SFD_TH_PRF64, pports);
    }
}
 ...
 * no return value
 */
void dwt_configure(dwt_config_t *config, UWBPortTypeDef *pports)
{
    ...
    dwt_write32bitreg(SYS_CFG_ID,pdw1000local->sysCFGreg, pports) ;
    // Set the lde_replicaCoeff
    dwt_write16bitoffsetreg(LDE_IF_ID, LDE_REPC_OFFSET, reg16, pports) ;

    _dwt_configlde(prfIndex, pports);

    // Configure PLL2/RF PLL block CFG/TUNE (for a given channel)
    dwt_write32bitoffsetreg(FS_CTRL_ID, FS_PLLCFG_OFFSET, fs_pll_cfg[chan_idx[chan]], pports);
    dwt_write8bitoffsetreg(FS_CTRL_ID, FS_PLLTUNE_OFFSET, fs_pll_tune[chan_idx[chan]], pports);

    // Configure Pulse Generator Delay
    dwt_write8bitoffsetreg(TX_CAL_ID, TC_PGDELAY_OFFSET, tc_pgdelay[chan_idx[chan]], pports);

    // Configure RF RX blocks (for specified channel/bandwidth)
    dwt_write8bitoffsetreg(RF_CONF_ID, RF_RXCTRLH_OFFSET, rx_config[bw], pports);

    // Configure RF TX blocks (for specified channel and PRF)
    // Configure RF TX control
    dwt_write32bitoffsetreg(RF_CONF_ID, RF_TXCTRL_OFFSET, tx_config[chan_idx[chan]], pports);

    // Configure the baseband parameters (for specified PRF, bit rate, PAC, and SFD settings)
    // DTUNE0
    dwt_write16bitoffsetreg(DRX_CONF_ID, DRX_TUNE0b_OFFSET, sftsh[config->dataRate][config->nsSFD], pports);

    // DTUNE1
    dwt_write16bitoffsetreg(DRX_CONF_ID, DRX_TUNE1a_OFFSET, dtune1[prfIndex], pports);

    if(config->dataRate == DWT_BR_110K)
    {
        dwt_write16bitoffsetreg(DRX_CONF_ID, DRX_TUNE1b_OFFSET, DRX_TUNE1b_110K, pports);
    }
    else
    {
        if(config->txPreambLength == DWT_PLEN_64)
        {
            dwt_write16bitoffsetreg(DRX_CONF_ID, DRX_TUNE1b_OFFSET, DRX_TUNE1b_6M8_PRE64, pports);
            dwt_write8bitoffsetreg(DRX_CONF_ID, DRX_TUNE4H_OFFSET, DRX_TUNE4H_PRE64, pports);
        }
        else
        {
            dwt_write16bitoffsetreg(DRX_CONF_ID, DRX_TUNE1b_OFFSET, DRX_TUNE1b_850K_6M8, pports);
            dwt_write8bitoffsetreg(DRX_CONF_ID, DRX_TUNE4H_OFFSET, DRX_TUNE4H_PRE128PLUS, pports);
        }
    }

    // DTUNE2
    dwt_write32bitoffsetreg(DRX_CONF_ID, DRX_TUNE2_OFFSET, digital_bb_config[prfIndex][config->rxPAC], pports);
    ...
    dwt_write16bitoffsetreg(DRX_CONF_ID, DRX_SFDTOC_OFFSET, config->sfdTO, pports);

    // Configure AGC parameters
    dwt_write32bitoffsetreg( AGC_CFG_STS_ID, 0xC, agc_config.lo32, pports);
    dwt_write16bitoffsetreg( AGC_CFG_STS_ID, 0x4, agc_config.target[prfIndex], pports);

    // Set (non-standard) user SFD for improved performance,
    if(config->nsSFD)
    {
        // Write non standard (DW) SFD length
        dwt_write8bitoffsetreg(USR_SFD_ID, 0x00, dwnsSFDlen[config->dataRate], pports);
        nsSfd_result = 3 ;
        useDWnsSFD = 1 ;
    }
    ...
    dwt_write32bitreg(CHAN_CTRL_ID,regval, pports) ;

    // Set up TX Preamble Size, PRF and Data Rate
    pdw1000local->txFCTRL = ((uint32)(config->txPreambLength | config->prf) << TX_FCTRL_TXPRF_SHFT) | ((uint32)config->dataRate << TX_FCTRL_TXBR_SHFT);
    dwt_write32bitreg(TX_FCTRL_ID, pdw1000local->txFCTRL, pports);

    // The SFD transmit pattern is initialised by the DW1000 upon a user TX request, but (due to an IC issue) it is not done for an auto-ACK TX. The
    // SYS_CTRL write below works around this issue, by simultaneously initiating and aborting a transmission, which correctly initialises the SFD
    // after its configuration or reconfiguration.
    // This issue is not documented at the time of writing this code. It should be in next release of DW1000 User Manual (v2.09, from July 2016).
    dwt_write8bitoffsetreg(SYS_CTRL_ID, SYS_CTRL_OFFSET, SYS_CTRL_TXSTRT | SYS_CTRL_TRXOFF, pports); // Request TX start and TRX off at the same time
} // end dwt_configure()
 ...
 * no return value
 */
void dwt_setrxantennadelay(uint16 rxDelay, UWBPortTypeDef *pports)
{
    // Set the RX antenna delay for auto TX timestamp adjustment
    dwt_write16bitoffsetreg(LDE_IF_ID, LDE_RXANTD_OFFSET, rxDelay, pports);
}
 ...
 * no return value
 */
void dwt_settxantennadelay(uint16 txDelay, UWBPortTypeDef *pports)
{
    // Set the TX antenna delay for auto TX timestamp adjustment
    dwt_write16bitoffsetreg(TX_ANTD_ID, TX_ANTD_OFFSET, txDelay, pports);
}
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
int dwt_writetxdata(uint16 txFrameLength, uint8 *txFrameBytes, uint16 txBufferOffset, UWBPortTypeDef *pports)
{
#ifdef DWT_API_ERROR_CHECK
    ...
    if ((txBufferOffset + txFrameLength) <= 1024)
    {
        ...
        dwt_writetodevice( TX_BUFFER_ID, txBufferOffset, txFrameLength-2, txFrameBytes, pports);  //看到了自动减去2
        return DWT_SUCCESS;
    }
    else
    {
 ...
 * no return value
 */
void dwt_writetxfctrl(uint16 txFrameLength, uint16 txBufferOffset, int ranging, UWBPortTypeDef *pports)
{
    ...
    dwt_write32bitreg(TX_FCTRL_ID, reg32, pports);
 ...
 * no return value
 */
void dwt_readrxdata(uint8 *buffer, uint16 length, uint16 rxBufferOffset, UWBPortTypeDef *pports)
{
    dwt_readfromdevice(RX_BUFFER_ID,rxBufferOffset,length,buffer,pports) ;
}
 ...
 * no return value
 */
void dwt_readaccdata(uint8 *buffer, uint16 len, uint16 accOffset, UWBPortTypeDef *pports)
{
    // Force on the ACC clocks if we are sequenced
    _dwt_enableclocks(READ_ACC_ON, pports);

    dwt_readfromdevice(ACC_MEM_ID,accOffset,len,buffer,pports) ;

    _dwt_enableclocks(READ_ACC_OFF, pports); // Revert clocks back
}
...
int32 dwt_readcarrierintegrator(UWBPortTypeDef *pports)
{
    uint32  regval = 0 ;
    int     j ;
    ...
    dwt_readfromdevice(DRX_CONF_ID,DRX_CARRIER_INT_OFFSET,DRX_CARRIER_INT_LEN, buffer, pports) ;
 ...
 * no return value
 */
void dwt_readdiagnostics(dwt_rxdiag_t *diagnostics, UWBPortTypeDef *pports)
{
    // Read the HW FP index
    diagnostics->firstPath = dwt_read16bitoffsetreg(RX_TIME_ID, RX_TIME_FP_INDEX_OFFSET, pports);

    // LDE diagnostic data
    diagnostics->maxNoise = dwt_read16bitoffsetreg(LDE_IF_ID, LDE_THRESH_OFFSET, pports);

    // Read all 8 bytes in one SPI transaction
    dwt_readfromdevice(RX_FQUAL_ID, 0x0, 8, (uint8*)&diagnostics->stdNoise, pports);

    diagnostics->firstPathAmp1 = dwt_read16bitoffsetreg(RX_TIME_ID, RX_TIME_FP_AMPL1_OFFSET, pports);

    diagnostics->rxPreamCount = (dwt_read32bitreg(RX_FINFO_ID, pports) & RX_FINFO_RXPACC_MASK) >> RX_FINFO_RXPACC_SHIFT  ;

    diagnostics->rxPreamCountNOSAT = dwt_read16bitoffsetreg(DRX_CONF_ID, RXPACC_NOSAT_OFFSET, pports);
}
 ...
 * no return value
 */
void dwt_readtxtimestamp(uint8 * timestamp, UWBPortTypeDef *pports)
{
    dwt_readfromdevice(TX_TIME_ID, TX_TIME_TX_STAMP_OFFSET, TX_TIME_TX_STAMP_LEN, timestamp, pports) ; // Read bytes directly into buffer
}
 ...
 */
uint32 dwt_readtxtimestamphi32(UWBPortTypeDef *pports)
{
    return dwt_read32bitoffsetreg(TX_TIME_ID, 1, pports); // Offset is 1 to get the 4 upper bytes out of 5
}
 ...
 */
uint32 dwt_readtxtimestamplo32(UWBPortTypeDef *pports)
{
    return dwt_read32bitreg(TX_TIME_ID, pports); // Read TX TIME as a 32-bit register to get the 4 lower bytes out of 5
}
 ...
 * no return value
 */
void dwt_readrxtimestamp(uint8 * timestamp, UWBPortTypeDef *pports)
{
    dwt_readfromdevice(RX_TIME_ID, RX_TIME_RX_STAMP_OFFSET, RX_TIME_RX_STAMP_LEN, timestamp, pports) ; // Get the adjusted time of arrival
}
 ...
 */
uint32 dwt_readrxtimestamphi32(UWBPortTypeDef *pports)
{
    return dwt_read32bitoffsetreg(RX_TIME_ID, 1, pports); // Offset is 1 to get the 4 upper bytes out of 5
}
 ...
 */
uint32 dwt_readrxtimestamplo32(UWBPortTypeDef *pports)
{
    return dwt_read32bitreg(RX_TIME_ID, pports); // Read RX TIME as a 32-bit register to get the 4 lower bytes out of 5
}
 ...
 */
uint32 dwt_readsystimestamphi32(UWBPortTypeDef *pports)
{
    return dwt_read32bitoffsetreg(SYS_TIME_ID, 1, pports); // Offset is 1 to get the 4 upper bytes out of 5
}
 ...
 * no return value
 */
void dwt_readsystime(uint8 * timestamp, UWBPortTypeDef *pports)
{
    dwt_readfromdevice(SYS_TIME_ID, SYS_TIME_OFFSET, SYS_TIME_LEN, timestamp, pports) ;
}
...
)
{
    ...
    // Write it to the SPI
    writetospi(cnt,header,length,buffer,pports);
...
)
{
    ...
    readfromspi(cnt, header, length, buffer, pports);  // result is stored in the buffer
 ...
 */
uint32 dwt_read32bitoffsetreg(int regFileID, int regOffset, UWBPortTypeDef *pports)
{
    uint32  regval = 0 ;
    int     j ;
    uint8   buffer[4] ;

    dwt_readfromdevice(regFileID,regOffset,4,buffer,pports); // Read 4 bytes (32-bits) register into buffer
 ...
 */
uint16 dwt_read16bitoffsetreg(int regFileID, int regOffset, UWBPortTypeDef *pports)
{
    uint16  regval = 0 ;
    uint8   buffer[2] ;

    dwt_readfromdevice(regFileID,regOffset,2,buffer,pports); // Read 2 bytes (16-bits) register into buffer
 ...
 */
uint8 dwt_read8bitoffsetreg(int regFileID, int regOffset, UWBPortTypeDef *pports)
{
    uint8 regval;

    dwt_readfromdevice(regFileID, regOffset, 1, &regval, pports);
 ...
 * no return value
 */
void dwt_write8bitoffsetreg(int regFileID, int regOffset, uint8 regval, UWBPortTypeDef *pports)
{
    dwt_writetodevice(regFileID, regOffset, 1, &regval, pports);
}
 ...
 * no return value
 */
void dwt_write16bitoffsetreg(int regFileID, int regOffset, uint16 regval, UWBPortTypeDef *pports)
{
    uint8   buffer[2] ;
    ...
    dwt_writetodevice(regFileID,regOffset,2,buffer,pports);
 ...
 * no return value
 */
void dwt_write32bitoffsetreg(int regFileID, int regOffset, uint32 regval, UWBPortTypeDef *pports)
{
    int     j ;
    uint8   buffer[4] ;
    ...
    dwt_writetodevice(regFileID,regOffset,4,buffer,pports);
 ...
 * no return value
 */
void dwt_enableframefilter(uint16 enable, UWBPortTypeDef *pports)
{
    uint32 sysconfig = SYS_CFG_MASK & dwt_read32bitreg(SYS_CFG_ID, pports) ; // Read sysconfig register
    ...
    dwt_write32bitreg(SYS_CFG_ID,sysconfig, pports) ;
 ...
 * no return value
 */
void dwt_setpanid(uint16 panID, UWBPortTypeDef *pports)
{
    // PAN ID is high 16 bits of register
    dwt_write16bitoffsetreg(PANADR_ID, PANADR_PAN_ID_OFFSET, panID, pports);
}
 ...
 * no return value
 */
void dwt_setaddress16(uint16 shortAddress, UWBPortTypeDef *pports)
{
    // Short address into low 16 bits
    dwt_write16bitoffsetreg(PANADR_ID, PANADR_SHORT_ADDR_OFFSET, shortAddress, pports);
}
 ...
 * no return value
 */
void dwt_seteui(uint8 *eui64, UWBPortTypeDef *pports)
{
    dwt_writetodevice(EUI_64_ID, EUI_64_OFFSET, EUI_64_LEN, eui64, pports);
}
 ...
 * no return value
 */
void dwt_geteui(uint8 *eui64, UWBPortTypeDef *pports)
{
    dwt_readfromdevice(EUI_64_ID, EUI_64_OFFSET, EUI_64_LEN, eui64, pports);
}
 ...
 * no return value
 */
void dwt_otpread(uint16 address, uint32 *array, uint8 length, UWBPortTypeDef *pports)
{
    int i;

    _dwt_enableclocks(FORCE_SYS_XTI, pports); // NOTE: Set system clock to XTAL - this is necessary to make sure the values read by _dwt_otpread are reliable

    for(i=0; i<length; i++)
    {
        array[i] = _dwt_otpread(address + i, pports) ;
    }

    _dwt_enableclocks(ENABLE_ALL_SEQ, pports); // Restore system clock to PLL
...
uint32 _dwt_otpread(uint16 address, UWBPortTypeDef *pports)
{
    ...
    // Write the address
    dwt_write16bitoffsetreg(OTP_IF_ID, OTP_ADDR, address, pports);

    // Perform OTP Read - Manual read mode has to be set
    dwt_write8bitoffsetreg(OTP_IF_ID, OTP_CTRL, OTP_CTRL_OTPREAD | OTP_CTRL_OTPRDEN, pports);
    dwt_write8bitoffsetreg(OTP_IF_ID, OTP_CTRL, 0x00, pports); // OTPREAD is self clearing but OTPRDEN is not

    // Read read data, available 40ns after rising edge of OTP_READ
    ret_data = dwt_read32bitoffsetreg(OTP_IF_ID, OTP_RDAT, pports);
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
uint32 _dwt_otpsetmrregs(int mode, UWBPortTypeDef *pports)
{
    uint8 wr_buf[4];
    ...
    // Set MRA, MODE_SEL
    wr_buf[0] = 0x03;
    dwt_writetodevice(OTP_IF_ID, OTP_CTRL+1,1,wr_buf, pports);
    ...
    dwt_writetodevice(OTP_IF_ID, OTP_WDAT,2,wr_buf, pports);


    // Set WRITE_MR
    wr_buf[0] = 0x08;
    dwt_writetodevice(OTP_IF_ID, OTP_CTRL,1,wr_buf, pports);
...
uint32 _dwt_otpprogword32(uint32 data, uint16 address, UWBPortTypeDef *pports)
{
    ...
    wr_buf[0] = data & 0xff;
    dwt_writetodevice(OTP_IF_ID, OTP_WDAT, 4, wr_buf, pports);

    // Write the address [10:0]
    wr_buf[1] = (address>>8) & 0x07;
    wr_buf[0] = address & 0xff;
    dwt_writetodevice(OTP_IF_ID, OTP_ADDR, 2, wr_buf, pports);
    ...
    {
        deca_sleep(1);
        dwt_readfromdevice(OTP_IF_ID, OTP_STAT, 1, rd_buf, pports);
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
int dwt_otpwriteandverify(uint32 value, uint16 address, UWBPortTypeDef *pports)
{
    int prog_ok = DWT_SUCCESS;
    int retry = 0;
    // Firstly set the system clock to crystal
    _dwt_enableclocks(FORCE_SYS_XTI, pports); //set system clock to XTI
    ...
    _dwt_otpsetmrregs(1, pports); // Set mode for programming

    // For each value to program - the readback/check is done couple of times to verify it has programmed successfully
    while(1)
    {
        _dwt_otpprogword32(value, address, pports);

        if(_dwt_otpread(address, pports) == value)
        {
            break;
        }
        retry++;
    ...
    _dwt_otpsetmrregs(4, pports); // Set mode for reading

    if(_dwt_otpread(address, pports) != value) // If this does not pass please check voltage supply on VDDIO
    {
        prog_ok = DWT_ERROR;
    }

    _dwt_otpsetmrregs(0, pports); // Setting OTP mode register for low RM read - resetting the device would be alternative
 ...
 * no return value
 */
void _dwt_aonconfigupload(UWBPortTypeDef *pports)
{
    dwt_write8bitoffsetreg(AON_ID, AON_CTRL_OFFSET, AON_CTRL_UPL_CFG, pports);
    dwt_write8bitoffsetreg(AON_ID, AON_CTRL_OFFSET, 0x00, pports); // Clear the register
}
...
{
    dwt_write8bitoffsetreg(AON_ID, AON_CTRL_OFFSET, 0x00, pports); // Clear the register
    dwt_write8bitoffsetreg(AON_ID, AON_CTRL_OFFSET, AON_CTRL_SAVE, pports);
}
 ...
 * no return value
 */
void dwt_entersleep(UWBPortTypeDef *pports)
{
    // Copy config to AON - upload the new configuration
    _dwt_aonarrayupload(pports);
}
 ...
 * no return value
 */
void dwt_configuresleepcnt(uint16 sleepcnt, UWBPortTypeDef *pports)
{
    // Force system clock to crystal
    _dwt_enableclocks(FORCE_SYS_XTI, pports);

    // Reset sleep configuration to make sure we don't accidentally go to sleep
    dwt_write8bitoffsetreg(AON_ID, AON_CFG0_OFFSET, 0x00, pports); // NB: this write change the default LPCLKDIVA value which is not used anyway.
    dwt_write8bitoffsetreg(AON_ID, AON_CFG1_OFFSET, 0x00, pports);

    // Disable the sleep counter
    _dwt_aonconfigupload(pports);

    // Set new value
    dwt_write16bitoffsetreg(AON_ID, AON_CFG0_OFFSET + AON_CFG0_SLEEP_TIM_OFFSET, sleepcnt, pports);
    _dwt_aonconfigupload(pports);

    // Enable the sleep counter
    dwt_write8bitoffsetreg(AON_ID, AON_CFG1_OFFSET, AON_CFG1_SLEEP_CEN, pports);
    _dwt_aonconfigupload(pports);

    // Put system PLL back on
    _dwt_enableclocks(ENABLE_ALL_SEQ, pports);
}
 ...
 */
uint16 dwt_calibratesleepcnt(UWBPortTypeDef *pports)
{
    uint16 result;

    // Enable calibration of the sleep counter
    dwt_write8bitoffsetreg(AON_ID, AON_CFG1_OFFSET, AON_CFG1_LPOSC_CAL, pports);
    _dwt_aonconfigupload(pports);
    ...
    dwt_write8bitoffsetreg(AON_ID, AON_ADDR_OFFSET, AON_ADDR_LPOSC_CAL_1, pports);

    // Enable manual override
    dwt_write8bitoffsetreg(AON_ID, AON_CTRL_OFFSET, AON_CTRL_DCA_ENAB, pports);

    // Read confirm data that was written
    dwt_write8bitoffsetreg(AON_ID, AON_CTRL_OFFSET, AON_CTRL_DCA_ENAB | AON_CTRL_DCA_READ, pports);

    // Read back byte from AON
    result = dwt_read8bitoffsetreg(AON_ID, AON_RDAT_OFFSET, pports);
    result <<= 8;

    // Set up address - Read lower byte
    dwt_write8bitoffsetreg(AON_ID, AON_ADDR_OFFSET, AON_ADDR_LPOSC_CAL_0, pports);
    ...
    dwt_write8bitoffsetreg(AON_ID, AON_CTRL_OFFSET, 0x00, pports);
 ...
 * no return value
 */
void dwt_configuresleep(uint16 mode, uint8 wake, UWBPortTypeDef *pports)
{
    // Add predefined sleep settings before writing the mode
    mode |= pdw1000local->sleep_mode;
    dwt_write16bitoffsetreg(AON_ID, AON_WCFG_OFFSET, mode, pports);

    dwt_write8bitoffsetreg(AON_ID, AON_CFG0_OFFSET, wake, pports);
}
 ...
 * no return value
 */
void dwt_entersleepaftertx(int enable, UWBPortTypeDef *pports)
{
    uint32 reg = dwt_read32bitoffsetreg(PMSC_ID, PMSC_CTRL1_OFFSET, pports);
    ...
    dwt_write32bitoffsetreg(PMSC_ID, PMSC_CTRL1_OFFSET, reg, pports);
}
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
int dwt_spicswakeup(uint8 *buff, uint16 length, UWBPortTypeDef *pports)
{
    if(dwt_readdevid(pports) != DWT_DEVICE_ID) // Device was in deep sleep (the first read fails)
    {
        // Need to keep chip select line low for at least 500us
        dwt_readfromdevice(0x0, 0x0, length, buff, pports); // Do a long read to wake up the chip (hold the chip select low)
    ...
    if(dwt_readdevid(pports) != DWT_DEVICE_ID)
    {
        return DWT_ERROR;
    }
 ...
 * no return value
 */
void _dwt_configlde(int prfIndex, UWBPortTypeDef *pports)
{
    dwt_write8bitoffsetreg(LDE_IF_ID, LDE_CFG1_OFFSET, LDE_PARAM1, pports); // 8-bit configuration register

    if(prfIndex)
    {
        dwt_write16bitoffsetreg( LDE_IF_ID, LDE_CFG2_OFFSET, (uint16) LDE_PARAM3_64, pports); // 16-bit LDE configuration tuning register
    }
    else
    {
        dwt_write16bitoffsetreg( LDE_IF_ID, LDE_CFG2_OFFSET, (uint16) LDE_PARAM3_16, pports);
    }
}
...
{
    // Set up clocks
    _dwt_enableclocks(FORCE_LDE, pports);

    // Kick off the LDE load
    dwt_write16bitoffsetreg(OTP_IF_ID, OTP_CTRL, OTP_CTRL_LDELOAD, pports); // Set load LDE kick bit
 ...
 * no return value
 */
void dwt_loadopsettabfromotp(uint8 ops_sel, UWBPortTypeDef *pports)
{
    ...
    dwt_write16bitoffsetreg(OTP_IF_ID, OTP_SF, reg, pports);
 ...
 * no return value
 */
void dwt_setsmarttxpower(int enable, UWBPortTypeDef *pports)
{
 ...
 * no return value
 */
void dwt_enableautoack(uint8 responseDelayTime, UWBPortTypeDef *pports)
{
    // Set auto ACK reply delay
    dwt_write8bitoffsetreg(ACK_RESP_T_ID, ACK_RESP_T_ACK_TIM_OFFSET, responseDelayTime, pports); // In symbols
    // Enable auto ACK
 ...
 * no return value
 */
void dwt_setdblrxbuffmode(int enable, UWBPortTypeDef *pports)
{
    if(enable)
    {
 ...
 * no return value
 */
void dwt_setrxaftertxdelay(uint32 rxDelayTime, UWBPortTypeDef *pports)
{
    uint32 val = dwt_read32bitreg(ACK_RESP_T_ID, pports) ; // Read ACK_RESP_T_ID register

    val &= ~(ACK_RESP_T_W4R_TIM_MASK) ; // Clear the timer (19:0)

    val |= (rxDelayTime & ACK_RESP_T_W4R_TIM_MASK) ; // In UWB microseconds (e.g. turn the receiver on 20uus after TX)

    dwt_write32bitreg(ACK_RESP_T_ID, val, pports) ;
}
 ...
 */
uint8 dwt_checkirq(UWBPortTypeDef *pports)
{
    return (dwt_read8bitoffsetreg(SYS_STATUS_ID, SYS_STATUS_OFFSET, pports) & SYS_STATUS_IRQS); // Reading the lower byte only is enough for this operation
}
 ...
 * no return value
 */
void dwt_isr(UWBPortTypeDef *pports)
{
    uint32 status = pdw1000local->cbData.status = dwt_read32bitreg(SYS_STATUS_ID, pports); // Read status register low 32bits
    ...
    if(status & SYS_STATUS_RXFCG)
    {
        ...
        dwt_write32bitreg(SYS_STATUS_ID, (SYS_STATUS_RXDFR | SYS_STATUS_RXFCG | SYS_STATUS_RXPRD | \
                SYS_STATUS_RXSFDD | SYS_STATUS_RXPHD ), pports); // Clear all receive status bits

        pdw1000local->cbData.rx_flags = 0;

        // Read frame info - Only the first two bytes of the register are used here.
        finfo16 = dwt_read16bitoffsetreg(RX_FINFO_ID, RX_FINFO_OFFSET, pports);
        ...
        dwt_readfromdevice(RX_BUFFER_ID, 0, FCTRL_LEN_MAX, pdw1000local->cbData.fctrl, pports);
        ...
        {
            dwt_write32bitreg(SYS_STATUS_ID, SYS_STATUS_AAT, pports); // Clear AAT status bit in register
        ...
        {
            pdw1000local->cbRxOk(&pdw1000local->cbData, pports);
        }

        if (pdw1000local->dblbuffon)
        {
            // Toggle the Host side Receive Buffer Pointer
            dwt_write8bitoffsetreg(SYS_CTRL_ID, SYS_CTRL_HRBT_OFFSET, 1, pports);
        }
    }
    ...
    {
        dwt_write32bitreg(SYS_STATUS_ID, SYS_STATUS_ALL_TX, pports); // Clear TX event bits  就是啊，这边会自动清除的啊，为什么还要什么东西呢？
    ...
    {
        dwt_write32bitreg(SYS_STATUS_ID, SYS_STATUS_ALL_RX_TO, pports); // Clear RX timeout event bits
        ...
        {
            pdw1000local->cbRxTo(&pdw1000local->cbData, pports);
        }
    }
    ...
    {
        dwt_write32bitreg(SYS_STATUS_ID, SYS_STATUS_ALL_RX_ERR, pports); // Clear RX error event bits
        ...
        {
            pdw1000local->cbRxErr(&pdw1000local->cbData, pports);
        }
    }
}
 ...
 * no return value
 */
void dwt_lowpowerlistenisr(UWBPortTypeDef *pports)
{
    uint32 status = pdw1000local->cbData.status = dwt_read32bitreg(SYS_STATUS_ID, pports); // Read status register low 32bits
    uint16 finfo16;
    uint16 len;
    ...
    dwt_setlowpowerlistening(0, pports);

    dwt_write32bitreg(SYS_STATUS_ID, SYS_STATUS_ALL_RX_GOOD, pports); // Clear all receive status bits

    pdw1000local->cbData.rx_flags = 0;

    // Read frame info - Only the first two bytes of the register are used here.
    finfo16 = dwt_read16bitoffsetreg(RX_FINFO_ID, 0, pports);
 ...
 * no return value
 */
void dwt_setleds(uint8 mode, UWBPortTypeDef *pports)
{
    uint32 reg;
    ...
    if (mode & DWT_LEDS_ENABLE)
    {
        ...
        reg = dwt_read32bitoffsetreg(GPIO_CTRL_ID, GPIO_MODE_OFFSET, pports);
        reg &= ~(GPIO_MSGP2_MASK | GPIO_MSGP3_MASK);
        reg |= (GPIO_PIN2_RXLED | GPIO_PIN3_TXLED);
        dwt_write32bitoffsetreg(GPIO_CTRL_ID, GPIO_MODE_OFFSET, reg, pports);
        ...
        reg = dwt_read32bitoffsetreg(PMSC_ID, PMSC_CTRL0_OFFSET, pports);
        reg |= (PMSC_CTRL0_GPDCE | PMSC_CTRL0_KHZCLEN);
        dwt_write32bitoffsetreg(PMSC_ID, PMSC_CTRL0_OFFSET, reg, pports);
        ...
        dwt_write32bitoffsetreg(PMSC_ID, PMSC_LEDC_OFFSET, reg, pports);
...
void _dwt_enableclocks(int clocks, UWBPortTypeDef *pports)
{
    ...
    dwt_readfromdevice(PMSC_ID, PMSC_CTRL0_OFFSET, 2, reg, pports);
    switch(clocks)
    {
        case ENABLE_ALL_SEQ:
        {
    ...
    dwt_writetodevice(PMSC_ID, PMSC_CTRL0_OFFSET, 1, &reg[0], pports);
    dwt_writetodevice(PMSC_ID, 0x1, 1, &reg[1], pports);
 ...
 * no return value
 */
void _dwt_disablesequencing(UWBPortTypeDef *pports) // Disable sequencing and go to state "INIT"
{
    _dwt_enableclocks(FORCE_SYS_XTI, pports); // Set system clock to XTI

    dwt_write16bitoffsetreg(PMSC_ID, PMSC_CTRL1_OFFSET, PMSC_CTRL1_PKTSEQ_DISABLE, pports); // Disable PMSC ctrl of RF and RX clk blocks
}
 ...
 * no return value
 */
void dwt_setdelayedtrxtime(uint32 starttime, UWBPortTypeDef *pports)
{
    dwt_write32bitoffsetreg(DX_TIME_ID, 1, starttime, pports); // Write at offset 1 as the lower 9 bits of this register are ignored
...
int dwt_starttx(uint8 mode, UWBPortTypeDef *pports)
{
    ...
    if (mode & DWT_START_TX_DELAYED)
    {
        ...
        dwt_write8bitoffsetreg(SYS_CTRL_ID, SYS_CTRL_OFFSET, temp, pports);
        checkTxOK = dwt_read16bitoffsetreg(SYS_STATUS_ID, 3, pports); // Read at offset 3 to get the upper 2 bytes out of 5
 ...
 * no return value
 */
void dwt_forcetrxoff(UWBPortTypeDef *pports)
{
    decaIrqStatus_t stat ;
    uint32 mask;

    mask = dwt_read32bitreg(SYS_MASK_ID, pports) ; // Read set interrupt mask
    ...
    dwt_write32bitreg(SYS_MASK_ID, 0, pports) ; // Clear interrupt mask - so we don't get any unwanted events

    dwt_write8bitoffsetreg(SYS_CTRL_ID, SYS_CTRL_OFFSET, (uint8)SYS_CTRL_TRXOFF, pports) ; // Disable the radio

    // Forcing Transceiver off - so we do not want to see any new events that may have happened
    dwt_write32bitreg(SYS_STATUS_ID, (SYS_STATUS_ALL_TX | SYS_STATUS_ALL_RX_ERR | SYS_STATUS_ALL_RX_TO | SYS_STATUS_ALL_RX_GOOD), pports);

    dwt_syncrxbufptrs(pports);

    dwt_write32bitreg(SYS_MASK_ID, mask, pports) ; // Set interrupt mask to what it was
 ...
 * no return value
 */
void dwt_syncrxbufptrs(UWBPortTypeDef *pports)
{
    uint8  buff ;
    // Need to make sure that the host/IC buffer pointers are aligned before starting RX
    buff = dwt_read8bitoffsetreg(SYS_STATUS_ID, 3, pports); // Read 1 byte at offset 3 to get the 4th byte out of 5

    if((buff & (SYS_STATUS_ICRBP >> 24)) !=     // IC side Receive Buffer Pointer
       ((buff & (SYS_STATUS_HSRBP>>24)) << 1) ) // Host Side Receive Buffer Pointer
    {
        dwt_write8bitoffsetreg(SYS_CTRL_ID, SYS_CTRL_HRBT_OFFSET , 0x01, pports) ; // We need to swap RX buffer status reg (write one to toggle internally)
    }
}
 ...
 * no return value
 */
void dwt_setsniffmode(int enable, uint8 timeOn, uint8 timeOff, UWBPortTypeDef *pports)
{
    uint32 pmsc_reg;
    if (enable)
    {
        ...
        dwt_write16bitoffsetreg(RX_SNIFF_ID, RX_SNIFF_OFFSET, sniff_reg, pports);
        pmsc_reg = dwt_read32bitoffsetreg(PMSC_ID, PMSC_CTRL0_OFFSET, pports);
        pmsc_reg |= PMSC_CTRL0_PLL2_SEQ_EN;
        dwt_write32bitoffsetreg(PMSC_ID, PMSC_CTRL0_OFFSET, pmsc_reg, pports);
    }
    else
    {
        /* Clear ON/OFF times and disable PLL2 on/off sequencing by SNIFF mode. */
        dwt_write16bitoffsetreg(RX_SNIFF_ID, RX_SNIFF_OFFSET, 0x0000, pports);
        pmsc_reg = dwt_read32bitoffsetreg(PMSC_ID, PMSC_CTRL0_OFFSET, pports);
        pmsc_reg &= ~PMSC_CTRL0_PLL2_SEQ_EN;
        dwt_write32bitoffsetreg(PMSC_ID, PMSC_CTRL0_OFFSET, pmsc_reg, pports);
    }
 ...
 * no return value
 */
void dwt_setlowpowerlistening(int enable, UWBPortTypeDef *pports)
{
    uint32 pmsc_reg = dwt_read32bitoffsetreg(PMSC_ID, PMSC_CTRL1_OFFSET, pports);
    if (enable)
    {
    ...
    dwt_write32bitoffsetreg(PMSC_ID, PMSC_CTRL1_OFFSET, pmsc_reg, pports);
}
 ...
 * no return value
 */
void dwt_setsnoozetime(uint8 snooze_time, UWBPortTypeDef *pports)
{
    dwt_write8bitoffsetreg(PMSC_ID, PMSC_SNOZT_OFFSET, snooze_time, pports);
}
 ...
 */
int dwt_rxenable(int mode, UWBPortTypeDef *pports)
{
    uint16 temp ;
    uint8 temp1 ;
    ...
    dwt_write16bitoffsetreg(SYS_CTRL_ID, SYS_CTRL_OFFSET, temp, pports);

    if (mode & DWT_START_RX_DELAYED) // check for errors
    {
        temp1 = dwt_read8bitoffsetreg(SYS_STATUS_ID, 3, pports); // Read 1 byte at offset 3 to get the 4th byte out of 5
        if ((temp1 & (SYS_STATUS_HPDWARN >> 24)) != 0) // if delay has passed do immediate RX on unless DWT_IDLE_ON_DLY_ERR is true
        {
            dwt_forcetrxoff(pports); // turn the delayed receive off

            if((mode & DWT_IDLE_ON_DLY_ERR) == 0) // if DWT_IDLE_ON_DLY_ERR not set then re-enable receiver
            {
                dwt_write16bitoffsetreg(SYS_CTRL_ID, SYS_CTRL_OFFSET, SYS_CTRL_RXENAB, pports);
            }
 ...
 * no return value
 */
void dwt_setrxtimeout(uint16 time, UWBPortTypeDef *pports)
{
    uint8 temp ;

    temp = dwt_read8bitoffsetreg(SYS_CFG_ID, 3, pports); // Read at offset 3 to get the upper byte only

    if(time > 0)
    {
        dwt_write16bitoffsetreg(RX_FWTO_ID, RX_FWTO_OFFSET, time, pports) ;

        temp |= (uint8)(SYS_CFG_RXWTOE>>24); // Shift RXWTOE mask as we read the upper byte only
        // OR in 32bit value (1 bit set), I know this is in high byte.
        pdw1000local->sysCFGreg |= SYS_CFG_RXWTOE;

        dwt_write8bitoffsetreg(SYS_CFG_ID, 3, temp, pports); // Write at offset 3 to write the upper byte only
    }
    else
    {
        temp &= ~((uint8)(SYS_CFG_RXWTOE>>24)); // Shift RXWTOE mask as we read the upper byte only
 ...
 * no return value
 */
void dwt_setpreambledetecttimeout(uint16 timeout, UWBPortTypeDef *pports)
{
    dwt_write16bitoffsetreg(DRX_CONF_ID, DRX_PRETOC_OFFSET, timeout, pports);
}
 ...
 * no return value
 */
void dwt_setinterrupt(uint32 bitmask, uint8 operation, UWBPortTypeDef *pports)
{
    decaIrqStatus_t stat ;
    uint32 mask ;
    ...
    if(operation == 2)
    {
        dwt_write32bitreg(SYS_MASK_ID, bitmask, pports) ; // New value
    }
    else
    {
        mask = dwt_read32bitreg(SYS_MASK_ID, pports) ; // Read register
        if(operation == 1)
        {
            mask |= bitmask ;
        }
        ...
        dwt_write32bitreg(SYS_MASK_ID, mask, pports) ; // New value
    }
 ...
 * no return value
 */
void dwt_configeventcounters(int enable, UWBPortTypeDef *pports)
{
    // Need to clear and disable, can't just clear
    dwt_write8bitoffsetreg(DIG_DIAG_ID, EVC_CTRL_OFFSET, (uint8)(EVC_CLR), pports);

    if(enable)
    {
        dwt_write8bitoffsetreg(DIG_DIAG_ID, EVC_CTRL_OFFSET, (uint8)(EVC_EN), pports); // Enable
    }
}
 ...
 * no return value
 */
void dwt_readeventcounters(dwt_deviceentcnts_t *counters, UWBPortTypeDef *pports)
{
    uint32 temp;

    temp= dwt_read32bitoffsetreg(DIG_DIAG_ID, EVC_PHE_OFFSET, pports); // Read sync loss (31-16), PHE (15-0)
    counters->PHE = temp & 0xFFF;
    counters->RSL = (temp >> 16) & 0xFFF;

    temp = dwt_read32bitoffsetreg(DIG_DIAG_ID, EVC_FCG_OFFSET, pports); // Read CRC bad (31-16), CRC good (15-0)
    counters->CRCG = temp & 0xFFF;
    counters->CRCB = (temp >> 16) & 0xFFF;

    temp = dwt_read32bitoffsetreg(DIG_DIAG_ID, EVC_FFR_OFFSET, pports); // Overruns (31-16), address errors (15-0)
    counters->ARFE = temp & 0xFFF;
    counters->OVER = (temp >> 16) & 0xFFF;

    temp = dwt_read32bitoffsetreg(DIG_DIAG_ID, EVC_STO_OFFSET, pports); // Read PTO (31-16), SFDTO (15-0)
    counters->PTO = (temp >> 16) & 0xFFF;
    counters->SFDTO = temp & 0xFFF;

    temp = dwt_read32bitoffsetreg(DIG_DIAG_ID, EVC_FWTO_OFFSET, pports); // Read RX TO (31-16), TXFRAME (15-0)
    counters->TXF = (temp >> 16) & 0xFFF;
    counters->RTO = temp & 0xFFF;

    temp = dwt_read32bitoffsetreg(DIG_DIAG_ID, EVC_HPW_OFFSET, pports); // Read half period warning events
 ...
 * no return value
 */
void dwt_rxreset(UWBPortTypeDef *pports)
{
    // Set RX reset
    dwt_write8bitoffsetreg(PMSC_ID, PMSC_CTRL0_SOFTRESET_OFFSET, PMSC_CTRL0_RESET_RX, pports);

    // Clear RX reset
    dwt_write8bitoffsetreg(PMSC_ID, PMSC_CTRL0_SOFTRESET_OFFSET, PMSC_CTRL0_RESET_CLEAR, pports);
}
 ...
 * no return value
 */
void dwt_softreset(UWBPortTypeDef *pports)
{
    _dwt_disablesequencing(pports);

    // Clear any AON auto download bits (as reset will trigger AON download)
    dwt_write16bitoffsetreg(AON_ID, AON_WCFG_OFFSET, 0x00, pports);
    // Clear the wake-up configuration
    dwt_write8bitoffsetreg(AON_ID, AON_CFG0_OFFSET, 0x00, pports);
    // Upload the new configuration
    _dwt_aonarrayupload(pports);

    // Reset HIF, TX, RX and PMSC (set the reset bits)
    dwt_write8bitoffsetreg(PMSC_ID, PMSC_CTRL0_SOFTRESET_OFFSET, PMSC_CTRL0_RESET_ALL, pports);
 ...
 * no return value
 */
void dwt_setxtaltrim(uint8 value, UWBPortTypeDef *pports)
{
    // The 3 MSb in this 8-bit register must be kept to 0b011 to avoid any malfunction.
    uint8 reg_val = (3 << 5) | (value & FS_XTALT_MASK);
    dwt_write8bitoffsetreg(FS_CTRL_ID, FS_XTALT_OFFSET, reg_val, pports);
}
 ...
 */
uint8 dwt_getxtaltrim(UWBPortTypeDef *pports)
{
    return (dwt_read8bitoffsetreg(FS_CTRL_ID, FS_XTALT_OFFSET, pports) & FS_XTALT_MASK);
}
 ...
 * no return value
 */
void dwt_configcwmode(uint8 chan, UWBPortTypeDef *pports)
{
#ifdef DWT_API_ERROR_CHECK
    assert((chan >= 1) && (chan <= 7) && (chan != 6));
#endif
    ...
    // Enable RF PLL
    //
    dwt_write32bitreg(RF_CONF_ID, RF_CONF_TXPLLPOWEN_MASK, pports); // Enable LDO and RF PLL blocks
    dwt_write32bitreg(RF_CONF_ID, RF_CONF_TXALLEN_MASK, pports); // Enable the rest of TX blocks

    //
    // Configure TX clocks
    //
    dwt_write8bitoffsetreg(PMSC_ID, PMSC_CTRL0_OFFSET, 0x22, pports);
    dwt_write8bitoffsetreg(PMSC_ID, 0x1, 0x07, pports);

    // Disable fine grain TX sequencing
    dwt_setfinegraintxseq(0, pports);

    // Configure CW mode
    dwt_write8bitoffsetreg(TX_CAL_ID, TC_PGTEST_OFFSET, TC_PGTEST_CW, pports);
}
 ...
 * no return value
 */
void dwt_configcontinuousframemode(uint32 framerepetitionrate, UWBPortTypeDef *pports)
{
    //
    ...
    // Configure TX clocks
    //
    _dwt_enableclocks(FORCE_SYS_PLL, pports);
    _dwt_enableclocks(FORCE_TX_PLL, pports);
    ...
    dwt_write32bitreg(DX_TIME_ID, framerepetitionrate, pports);

    //
    // Configure continuous frame TX
    //
    dwt_write8bitoffsetreg(DIG_DIAG_ID, DIAG_TMC_OFFSET, (uint8)(DIAG_TMC_TX_PSTM), pports); // Turn the tx power spectrum test mode - continuous sending of frames
}
 ...
 */
uint16 dwt_readtempvbat(uint8 fastSPI, UWBPortTypeDef *pports)
{
    uint8 wr_buf[2];
    uint8 vbat_raw;
    uint8 temp_raw;
    ...
    dwt_writetodevice(RF_CONF_ID,0x11,1,wr_buf, pports);

    wr_buf[0] = 0x0A; // Enable TLD Bias and ADC Bias
    dwt_writetodevice(RF_CONF_ID,0x12,1,wr_buf, pports);
    ...
    if(fastSPI == 1)
    {
        ...
        wr_buf[0] = 0x00;
        dwt_writetodevice(TX_CAL_ID, TC_SARL_SAR_C,1,wr_buf, pports);
        wr_buf[0] = 0x01; // Set SAR enable
        dwt_writetodevice(TX_CAL_ID, TC_SARL_SAR_C,1,wr_buf, pports);

        deca_sleep(1); // If using PLL clocks(and fast SPI rate) then this sleep is needed
        // Read voltage and temperature.
        dwt_readfromdevice(TX_CAL_ID, TC_SARL_SAR_LVBAT_OFFSET,2,wr_buf, pports);
    }
    else //change to a slow clock
    {
        _dwt_enableclocks(FORCE_SYS_XTI, pports); // NOTE: set system clock to XTI - this is necessary to make sure the values read are reliable
        // Reading All SAR inputs
        wr_buf[0] = 0x00;
        dwt_writetodevice(TX_CAL_ID, TC_SARL_SAR_C,1,wr_buf, pports);
        wr_buf[0] = 0x01; // Set SAR enable
 ...
 */
uint8 dwt_readwakeuptemp(UWBPortTypeDef *pports)
{
    return dwt_read8bitoffsetreg(TX_CAL_ID, TC_SARL_SAR_LTEMP_OFFSET, pports);
}
 ...
 */
uint8 dwt_readwakeupvbat(UWBPortTypeDef *pports)
{
    return dwt_read8bitoffsetreg(TX_CAL_ID, TC_SARL_SAR_LVBAT_OFFSET, pports);
}
 ...
 */
uint32 dwt_calcbandwidthtempadj(uint16 target_count, UWBPortTypeDef *pports)
{
    int i;
    ...
    old_pmsc_ctrl0 = dwt_read8bitoffsetreg(PMSC_ID, PMSC_CTRL0_OFFSET, pports);
    old_pmsc_ctrl1 = dwt_read16bitoffsetreg(PMSC_ID, PMSC_CTRL1_OFFSET, pports);
    old_rf_conf_txpow_mask = dwt_read32bitreg(RF_CONF_ID, pports);

    //  Set clock to XTAL
    dwt_write8bitoffsetreg(PMSC_ID, PMSC_CTRL0_OFFSET, PMSC_CTRL0_SYSCLKS_19M, pports);

    //  Disable sequencing
    dwt_write16bitoffsetreg(PMSC_ID, PMSC_CTRL1_OFFSET, PMSC_CTRL1_PKTSEQ_DISABLE, pports);

    //  Turn on CLK PLL, Mix Bias and PG
    dwt_write32bitreg(RF_CONF_ID, RF_CONF_TXPOW_MASK | RF_CONF_PGMIXBIASEN_MASK, pports);

    //  Set sys and TX clock to PLL
    dwt_write8bitoffsetreg(PMSC_ID, PMSC_CTRL0_OFFSET, PMSC_CTRL0_SYSCLKS_125M | PMSC_CTRL0_TXCLKS_125M, pports);
    ...
    for (i = 0; i < 7; i++)
    {
        ...
        dwt_write8bitoffsetreg(TX_CAL_ID, TC_PGDELAY_OFFSET, curr_bw, pports);

        // Set cal direction and time
        dwt_write8bitoffsetreg(TX_CAL_ID, TC_PGCCTRL_OFFSET, TC_PGCCTRL_DIR_CONV | TC_PGCCTRL_TMEAS_MASK, pports);

        // Start cal
        dwt_write8bitoffsetreg(TX_CAL_ID, TC_PGCCTRL_OFFSET, TC_PGCCTRL_DIR_CONV | TC_PGCCTRL_TMEAS_MASK | TC_PGCCTRL_CALSTART, pports);
        // Allow cal to complete
        deca_sleep(1);

        // Read count value from the PG cal block
        raw_count = dwt_read16bitoffsetreg(TX_CAL_ID, TC_PGCAL_STATUS_OFFSET, pports) & TC_PGCAL_STATUS_DELAY_MASK;
    ...
    dwt_write8bitoffsetreg(PMSC_ID, PMSC_CTRL0_OFFSET, old_pmsc_ctrl0, pports);
    dwt_write16bitoffsetreg(PMSC_ID, PMSC_CTRL1_OFFSET, old_pmsc_ctrl1, pports);
    dwt_write32bitreg(RF_CONF_ID, old_rf_conf_txpow_mask, pports);
 ...
 */
uint16 dwt_calcpgcount(uint8 pgdly, UWBPortTypeDef *pports)
{
    ...
    for(i = 0; i < NUM_SAMPLES; i++) {
        // Write bw setting to PG_DELAY register
        dwt_write8bitoffsetreg(TX_CAL_ID, TC_PGDELAY_OFFSET, pgdly, pports);
        ...
        // Read count value from the PG cal block
        count = dwt_read16bitoffsetreg(TX_CAL_ID, TC_PGCAL_STATUS_OFFSET, pports) & TC_PGCAL_STATUS_DELAY_MASK;
...
void dwt_readrcphase(uint8_t *phase, UWBPortTypeDef *pports)
{
	uint64_t temp={0};
	dwt_readfromdevice(RX_TTCKO_ID, 0, RX_TTCKO_LEN, (uint8_t*)&temp, pports);

	*phase = (temp & RX_TTCKO_RCPHASE_MASK) >> 32;
}
// set EC_CTRL OSTRM (One Shot Timebase Reset)
void dwt_setecctrl(uint32_t reg, UWBPortTypeDef *pports)
{
	dwt_writetodevice(EXT_SYNC_ID, EC_CTRL_OFFSET, EC_CTRL_LEN, &reg, pports);
}
